    ...
    state.curr match
      case None =>
        if data.size < `--bound`.size then
          IO.succeed(state.copy(body = data))
        else if data startsWith `--bound--` then
          IO.succeed(HttpState.MsgDone(state.meta, BodyForm(state.form)))
        else if data startsWith `--bound` then
          data.indexOfSlice(`\r\n\r\n`) match
            ...
            case  i =>
              ...
              for
                headers <- IO.succeed(String(headersChunk, "utf8").split("\r\n").nn.drop(1).map(_.nn))
                s       <- 
      ...
      case Some(param: FormData.Param) =>
        data.indexOfSlice(`\r\n--bound`) match
          case -1 =>
            ...
            IO.succeed(state.copy(body = tail, curr = Some(p)))
          case  i => 
...
private def appendFile(file: FormData.File, value: Array[Byte]): ZIO[Blocking, Exception, Unit] =
  effectBlocking(JFiles.write(JPaths.get(file.path.toString), value, StandardOpenOption.APPEND)).unit.refineToOrDie[Exception]
