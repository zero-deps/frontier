...
final class DatagramChannel private[channels] (override protected[channels] val channel: JDatagramChannel)
  ...
  def localAddress: IO[IOException, Option[SocketAddress]] =
    ZIO.attempt(channel.getLocalAddress().nn).refineToOrDie[IOException].map(a => Option(a).map(new SocketAddress(_)))
  ...
  def receive(dst: ByteBuffer): IO[IOException, Option[SocketAddress]] =
    ZIO.attempt(channel.receive(dst.byteBuffer).nn).refineToOrDie[IOException].map(a => Option(a).map(new SocketAddress(_)))
  ...
  def remoteAddress: IO[IOException, Option[SocketAddress]] =
    ZIO.attempt(channel.getRemoteAddress().nn).refineToOrDie[IOException].map(a => Option(a).map(new SocketAddress(_)))
