...
final class MappedByteBuffer private[nio] (javaBuffer: JMappedByteBuffer) extends ByteBuffer(javaBuffer) {
  ...
  def load: IO[Nothing, Unit] = ZIO.environmentWithZIO(_.get.blocking(ZIO.succeed(javaBuffer.load()).unit))

  def force: IO[Nothing, Unit] = ZIO.environmentWithZIO(_.get.blocking(ZIO.succeed(javaBuffer.force()).unit))
}
