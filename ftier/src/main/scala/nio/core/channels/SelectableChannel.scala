...
object SocketChannel {
  ...
  final val open: IO[IOException, SocketChannel] =
    ZIO.attempt(new SocketChannel(JSocketChannel.open())).refineToOrDie[IOException]

  final def open(remote: SocketAddress): IO[IOException, SocketChannel] =
    ZIO.attempt(new SocketChannel(JSocketChannel.open(remote.jSocketAddress))).refineToOrDie[IOException]
}
...
object ServerSocketChannel {
  ...
  final val open: IO[IOException, ServerSocketChannel] =
    ZIO.attempt(new ServerSocketChannel(JServerSocketChannel.open())).refineToOrDie[IOException]
