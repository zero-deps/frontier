...
object FileChannel {
  ...
  def open(
    ...
    effectBlocking(new FileChannel(JFileChannel.open(path.javaPath, options.asJava, attrs *)))
      .refineToOrDie[Exception]

  def open(path: Path, options: OpenOption*): ZIO[Blocking, Exception, FileChannel] =
    effectBlocking(new FileChannel(JFileChannel.open(path.javaPath, options *)))
      .refineToOrDie[Exception]
