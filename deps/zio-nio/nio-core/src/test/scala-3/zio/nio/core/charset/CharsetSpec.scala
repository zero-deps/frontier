  ...
  override def spec =
    suite("CharsetSpec")(
      ...
      test("minimum buffer size for encoding") {
        ...
        assertM(in.transduce(t).runDrain.exit)(dies(isSubtype[IllegalArgumentException](anything)))
      },
      ...
      test("handles encoding errors") {
        ...
        assertM(in.transduce(t).runDrain.exit)(fails(isSubtype[UnmappableCharacterException](anything)))
      },
      test("handles decoding errors") {
        ...
        assertM(in.transduce(t).runDrain.exit)(fails(isSubtype[MalformedInputException](anything)))
      }
    )
